/*
 * Just the range coder code cribbed from compressjs.
 *
 * https://github.com/cscott/compressjs
 * Specifically subpath: /blob/master/lib/RangeCoder.js
 *
 * I guess this means the binjs-ts code needs to be made
 * GPL, which is fine by me, probably Dominic too.
 */
...
const CODE_BITS = 32;
const Top_value = Math.pow(2, CODE_BITS-1);
const SHIFT_BITS = (CODE_BITS - 9);
const EXTRA_BITS = ((CODE_BITS-2) % 8 + 1);
const Bottom_value = (Top_value >>> 8);

const MAX_INT = Math.pow(2, CODE_BITS) - 1;
...
export function RangeCoder(stream) {
...
function enc_normalize(rc, outputStream) {
...
function dec_normalize(rc, inputStream) {
